...
<head>
    ...
    <meta name="description" content="{{ $description ?? 'iReka Soft builds the Orderla commerce suite — WhatsApp ordering, storefronts, and F&B point of sale — from Cyberjaya, Malaysia.' }}">
...
<body @class([
    ...
    <header @class([
        'z-50',
        'border-b border-ink/10' => ! $immersive,
        'absolute inset-x-0 top-0 border-b border-white/10 bg-black/40 backdrop-blur-sm' => $immersive,
    ])>
        <div class="mx-auto max-w-7xl px-6 py-5 flex items-center justify-between gap-6">
            <a href="{{ route('home') }}" class="flex items-center gap-3 shrink-0 group">
                ...
                <span class="flex flex-col">
                    ...
                    ireka<span class="text-gold">soft</span>
                </span>
                    @if ($immersive)
                        <span class="mt-1 font-mono text-[10px] uppercase tracking-[0.18em] text-white/45 hidden sm:block">
                ...
                @foreach ($navItems as $index => $item)
                    ...
                    <a href="{{ route($item['route']) }}"
                       @class([
                           ...
                           request()->routeIs($item['route']) ? ($immersive ? 'text-white' : 'text-ink') : '',
                       ])>{{ $item['label'] }}</a>
                @endforeach
            </nav>
            ...
            <div class="flex items-center gap-2 sm:gap-3 shrink-0">
                <a href="{{ $ctaHref }}"
                   @if ($ctaExternal) target="_blank" rel="noopener" @endif
                   @class([
                ...
                >
                    <span class="mobile-menu-bar absolute block h-0.5 w-5 bg-current transition-all duration-300 -translate-y-1.5"></span>
    ...
    >
        ...
        <div class="relative z-10 flex h-full flex-col px-6 pt-6 pb-10">
                ...
                @foreach ($navItems as $item)
                    <a href="{{ route($item['route']) }}"
                       @class([
                           ...
                           request()->routeIs($item['route']) ? 'text-gold' : 'text-paper',
                       ])>{{ $item['label'] }}</a>
                @endforeach
            </nav>
            ...
            <a href="{{ $ctaHref }}"
               @if ($ctaExternal) target="_blank" rel="noopener" @endif
               class="mt-auto font-mono text-[13px] uppercase tracking-[0.12em] bg-gold text-paper px-6 py-3.5 rounded-sm text-center hover:bg-gold-soft transition-colors">
                {{ $ctaLabel }}
            </a>
        </div>
    </div>
        ...
        <div class="mx-auto max-w-6xl px-6 py-14 grid gap-10 md:grid-cols-4">
            <div class="md:col-span-2">
                ...
                <p class="mt-3 text-sm leading-relaxed max-w-sm text-paper/60">
                    A product studio in Cyberjaya, Malaysia, building commerce software under the Orderla name —
                    used by merchants from neighbourhood kedai to growing F&amp;B chains.
                </p>
            ...
            <div>
                ...
                <ul class="space-y-2 text-sm">
                    <li><a href="https://orderla.my" target="_blank" rel="noopener" class="hover:text-paper transition-colors">Orderla.my</a></li>
                    <li><a href="https://orderla.co" target="_blank" rel="noopener" class="hover:text-paper transition-colors">Orderla.co</a></li>
                    <li><a href="{{ route('products') }}" class="hover:text-paper transition-colors">Orderla FOS</a></li>
                </ul>
            </div>
            ...
            <div>
                ...
                <ul class="space-y-2 text-sm">
                    <li><a href="mailto:hello@irekasoft.com" class="hover:text-paper transition-colors">hello@irekasoft.com</a></li>
                    <li><a href="https://wa.me/60163373081" target="_blank" rel="noopener" class="hover:text-paper transition-colors">+60 16-337 3081</a></li>
